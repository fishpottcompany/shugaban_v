...
if(isset($free) && intval($free) > 0){
    ...
    if(isset($_GET["pll"]) && trim($_GET["pll"]) != ""){
        ...
        $query = $queryController->prepareAndExecuteQuery($mysqli, "SELECT id, name, portrait, year, category_id, plot, views, created_at FROM " . MOVIES_TABLE . " WHERE free = ? " . $added_condition . " ORDER by created_at DESC LIMIT 6", 2, "is", array($keyword, $max_created_at));

    } else {
        $max_created_at = "";
        $added_condition = "";
        $query = $queryController->prepareAndExecuteQuery($mysqli, "SELECT id, name, portrait, year, category_id, plot, views, created_at FROM " . MOVIES_TABLE . " WHERE free = ? " . $added_condition . " ORDER by created_at DESC LIMIT 6", 1, "i", array($keyword));
    }
} else {
    $nex_pg_keyword = "";
