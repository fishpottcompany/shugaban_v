...
$free = "-1";
if(isset($free) && intval($free) > 0){
    $nex_pg_keyword = "s=" . strval(intval($free))  . "&";
    $keyword = strval(intval($free));
    if(isset($_GET["pll"]) && trim($_GET["pll"]) != ""){
        $max_created_at = trim($_GET["pll"]);
        $added_condition = " AND created_at <= ? ";
        $query = $queryController->prepareAndExecuteQuery($mysqli, "SELECT id, name, portrait, year, category_id, plot, views, created_at FROM " . MOVIES_TABLE . " WHERE views > ? " . $added_condition . " views by created_at DESC LIMIT 6", 2, "is", array($keyword, $max_created_at));

    } else {
        $max_created_at = "";
        $added_condition = "";
        $query = $queryController->prepareAndExecuteQuery($mysqli, "SELECT id, name, portrait, year, category_id, plot, views, created_at FROM " . MOVIES_TABLE . " WHERE views > ? " . $added_condition . " views by created_at DESC LIMIT 6", 1, "i", array($keyword));
    }
} else {
    $nex_pg_keyword = "";
    if(isset($_GET["pll"]) && intval($_GET["pll"]) != ""){
        ...
        $query = $queryController->prepareAndExecuteQuery($mysqli, "SELECT id, name, portrait, year, category_id, plot, views, created_at FROM " . MOVIES_TABLE . " " . $added_condition . "   ORDER by views DESC LIMIT 6", 1, "s", array($max_created_at));

    } else {
        $max_created_at = "";
        $added_condition = "";
        $query = $queryController->prepareAndExecuteQuery($mysqli, "SELECT id, name, portrait, year, category_id, plot, views, created_at FROM " . MOVIES_TABLE . " ORDER by views DESC LIMIT 6", 0, "", "");
    }
}
                            ...
                            <div class="mb-4">
                                <div class="home-section mb-5">
                                    <header class="d-md-flex align-items-center justify-content-between mb-3 mb-lg-1 pb-2 w-100 border-bottom border-gray-3900">
                                        <h6 class="d-block position-relative font-size-24 font-weight-medium overflow-md-hidden m-0 text-white">Popular Movies</h6>
                                    </header>
